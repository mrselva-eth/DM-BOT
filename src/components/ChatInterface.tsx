...
export default function Component() {
    ...
    if (messageToUpdate && !messageToUpdate.isUser) {
      if ((isLike && likedMessages.has(id)) || (!isLike && dislikedMessages.has(id))) {
        setMessages(prev => prev.map(m => 
          m.id === id ? { ...m, likes: isLike ? (m.likes || 1) - 1 : m.likes, dislikes: !isLike ? (m.dislikes || 1) - 1 : m.dislikes } : m
        ))
        if (isLike) {
        ...
        setMessages(prev => prev.map(m => 
          m.id === id ? { 
            ...
            dislikes: !isLike ? (m.dislikes || 0) + 1 : dislikedMessages.has(id) ? (m.dislikes || 1) - 1 : m.dislikes 
          } : m
        ))
        if (isLike) {
  ...
  const handleWeatherSearch = async () => {
    try {
      const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${weatherCity}&appid=089824911a7c2ae5ffe699f59a2c1e53&units=metric`)
      if (!response.ok) {
  ...
  const highlightText = (text: string, query: string) => {
    ...
    return parts.map((part, index) => 
      part.toLowerCase() === query.toLowerCase() ? 
        `<span class="bg-green-300">${part}</span>` : part
    ).join('');
  }
      ...
      <div className="bg-blue-500 text-white p-2 flex justify-between items-center">
        ...
        {interfaceMode === 'chat' && (
          <div className="flex items-center search-container">
            ...
            <button onClick={() => setShowSearchInput(!showSearchInput)} className="bg-blue-600 p-1 rounded ml-2 search-button">
              <FaSearch />
            </button>
          </div>
        )}
                ...
                <div className={`max-w-[70%] p-2 rounded-lg ${
                  ...
                  {editingMessageId === message.id ? (
                    ...
                    <div className="mb-1 break-words">
                      {message.isUser ? (
                        <>
                          <span dangerouslySetInnerHTML={{ __html: makeLinkClickable(highlightText(message.text, searchQuery)) }} />
                          {message.isEdited && <span className="text-xs ml-1">(edited)</span>}
                        </>
                      ) : (
                        <TypingAnimation text={makeLinkClickable(highlightText(message.text, searchQuery))} />
                      )}
                    </div>
                  )}
